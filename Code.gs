...
function doPost(e) {
  try {
    ...
    else if (action === 'adminCreateVendor') result = adminCreateVendor(data.payload);
    else if (action === 'adminUpdateVendor') result = adminUpdateVendor(data.payload);
    ...
    // Admin & Vendor CRUD: Plans
    else if (action === 'crudPlan') result = crudPlan(data.payload);
    
    // Admin CRUD: Global Leads & Inquiries
    ...
    // Vendor Operations & Customer CRUD
...
function crudPlan(payload) {
  const sheet = SpreadsheetApp.openById(SHEET_ID).getSheetByName('Plans') || SpreadsheetApp.openById(SHEET_ID).insertSheet('Plans');
  if(sheet.getLastRow() === 0) sheet.appendRow(['Plan_ID', 'Name', 'Price', 'Features']);
...
function getAllLeadsAdmin() {
  ...
  if(!leadSheet) return { success: true, leads: [] };
...
function getVendorLeads(vendorId) {
  const leadSheet = SpreadsheetApp.openById(SHEET_ID).getSheetByName('Leads') || SpreadsheetApp.openById(SHEET_ID).insertSheet('Leads');
  if(leadSheet.getLastRow() === 0) leadSheet.appendRow(['Lead_ID', 'Vendor_ID', 'Customer_Name', 'Phone', 'Status', 'Notes', '', 'KW', 'Rate', 'Module_Brand', 'Module_Wp', 'Module_Qty', 'Inverter_Brand', 'Total_Amount', 'Date', 'Address', 'Geo']);
  const data = leadSheet.getDataRange().getValues();
  let leads = [];
  for(let i=1; i<data.length; i++) {
...
function generateQuotation(payload) {
  ...
  sheet.appendRow([
    leadId, payload.vendorId, payload.customer, payload.phone, 'Quotation Sent', payload.notes || '', '', 
...
function updateLead(payload) {
  ...
  for(let i=1; i<data.length; i++) {
    if(data[i][0] === payload.leadId) {
      ...
      sheet.getRange(i+1, 8).setValue(payload.kw);
      sheet.getRange(i+1, 9).setValue(payload.ratePerKw);
...
function handleTasks(payload) {
  const sheet = SpreadsheetApp.openById(SHEET_ID).getSheetByName('Tasks') || SpreadsheetApp.openById(SHEET_ID).insertSheet('Tasks');
  if(sheet.getLastRow() === 0) sheet.appendRow(['Task_ID', 'Vendor_ID', 'Lead_ID', 'Task_Name', 'Status', 'Due_Date']);
